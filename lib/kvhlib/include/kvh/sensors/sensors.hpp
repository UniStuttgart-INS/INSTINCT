...
class kvh_proglib_DLLEXPORT KvhImuSensor : private util::NoCopy
{
  public:
    /// \brief Defines a callback handler that can received notification when
    /// the KvhSensor receives raw data from its port.
    ///
    ...
    /// \brief Indicates if theKvhSensor is connected.
    ///
    ...
    /// \brief Connects to a kvh sensor.
    ///
    ...
    /// \brief Allows connecting to a kvh sensor over a \ref vn::common::ISimplePort.
    ///
    ...
    /// \brief Disconnects from the kvh sensor.
    void disconnect();
