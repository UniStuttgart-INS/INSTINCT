...
void LSM9DS1::ReadRegs(const char* dev, uint8_t ReadAddr, uint8_t* ReadBuf, unsigned int Bytes)
{
    ...
    if (!strcmp(dev, DEVICE_MAGNETOMETER))
        tx[0] |= MULTIPLE_READ;
...
bool LSM9DS1::initialize()
{
    ...
    WriteReg(DEVICE_ACC_GYRO, LSM9DS1XG_CTRL_REG1_G, BITS_ODR_G_952HZ | BITS_FS_G_2000DPS);
    usleep(200);

    // enable the three axes of the accelerometer
    WriteReg(DEVICE_ACC_GYRO, LSM9DS1XG_CTRL_REG5_XL, BITS_XEN_XL | BITS_YEN_XL | BITS_ZEN_XL);
    // configure the accelerometer-specify bandwidth selection with Abw
    WriteReg(DEVICE_ACC_GYRO, LSM9DS1XG_CTRL_REG6_XL, BITS_ODR_XL_952HZ | BITS_FS_XL_16G);
    usleep(200);
