...
#include "er/protocol/packetfinder.hpp"
...
class er_proglib_DLLEXPORT ErSensor : private util::NoCopy
{
  public:
    /// \brief Defines a callback handler that can received notification when
    /// the ErSensor receives raw data from its port.
    ///
    ...
    /// \brief Indicates if the ErSensor is connected.
    ///
