...
bool Packet::isValid()
{
    ...
    if (type() == TYPE_ASCII)
    {
        ...
        if (_data[_length - 5] == '*')
        {
            auto checksumHex = ubloxChecksum::checksumNMEA((char*)_data, _length);
            char checksumRecv[2];
            checksumRecv[0] = _data[_length - 4];
            checksumRecv[1] = _data[_length - 3];
            return to_uint8_from_hexstr(checksumRecv) == checksumHex;
        }
        else
        {
    ...
    {
        auto checksum = ubloxChecksum::checksumUBX(_data, _length);
