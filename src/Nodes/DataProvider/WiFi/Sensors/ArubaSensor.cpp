...
bool NAV::ArubaSensor::initialize()
{
    ...
    // authenticate
    if (ssh_userauth_password(_session, nullptr, _sshPassword.c_str()) != SSH_AUTH_SUCCESS)
    {
...
void NAV::ArubaSensor::readSensorDataThread(void* userData)
{
    ...
    // Read output
    do {
        bytesRead = static_cast<size_t>(ssh_channel_read_timeout(node->_channel, buffer.data(), sizeof(buffer), 0, 10)); // timeout in ms
        if (bytesRead > 0)
        {
    ...
    while (std::getline(iss, line) && !line.empty())
    {
        ...
        std::regex timeRegex(R"(\d{4}-\d{2}-\d{2})"); // Time format: YYYY-MM-DD
        ...
        lineStream >> value;
        timeStamp2 = value;
        double measuredDistance = static_cast<double>(rtt) * 1e-9 / 2 * InsConst::C_AIR;
        double measuredDistanceStd = static_cast<double>(stdValue) * 1e-9 / 2 * InsConst::C_AIR;
        if (std::regex_match(macAddress, macRegex)) // Check if the MAC address is valid
        {
            ...
            std::ranges::transform(macAddress, macAddress.begin(), ::toupper); // Convert to uppercase
