...
void NAV::Plot::plotFlowData(NAV::InputPin::NodeDataQueue& queue, size_t pinIdx)
{
    ...
    if (auto* sourcePin = inputPins.at(pinIdx).link.getConnectedPin())
    {
        ...
        // ---------------------------------------------- BARO -----------------------------------------------
        else if (NAV::NodeRegistry::NodeDataTypeAnyIsChildOf(sourcePin->dataIdentifier, { BaroPressObs::type() }))
        {
            plotData(std::static_pointer_cast<const BaroPressObs>(nodeData), pinIdx, i);
        }
        else if (NAV::NodeRegistry::NodeDataTypeAnyIsChildOf(sourcePin->dataIdentifier, { BaroHgt::type() }))
        {
            plotData(std::static_pointer_cast<const BaroHgt>(nodeData), pinIdx, i);
        }
        // --------------------------------------------- State -----------------------------------------------
