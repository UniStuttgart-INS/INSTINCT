...
void NAV::Plot::afterCreateLink(Pin* startPin, Pin* endPin)
{
    ...
    if (inputPins.at(pinIndex).type == Pin::Type::Flow)
    {
        ...
        else if (startPin->dataIdentifier.front() == ImuBiases::type())
        {
            ...
            data.at(pinIndex).addPlotDataItem("Accelerometer bias X accumulated [m/s^2]");
            data.at(pinIndex).addPlotDataItem("Accelerometer bias Y accumulated [m/s^2]");
            data.at(pinIndex).addPlotDataItem("Accelerometer bias Z accumulated [m/s^2]");
            data.at(pinIndex).addPlotDataItem("Gyroscope bias X accumulated [rad/s]");
            data.at(pinIndex).addPlotDataItem("Gyroscope bias Y accumulated [rad/s]");
            data.at(pinIndex).addPlotDataItem("Gyroscope bias Z accumulated [rad/s]");
        }
        ...
        else if (startPin->dataIdentifier.front() == SkydelObs::type())
        {
            ...
            data.at(pinIndex).addPlotDataItem("Y-ECEF [m]");
            data.at(pinIndex).addPlotDataItem("Z-ECEF [m]");
            data.at(pinIndex).addPlotDataItem("Roll [rad]");
            data.at(pinIndex).addPlotDataItem("Pitch [rad]");
            data.at(pinIndex).addPlotDataItem("Yaw [rad]");
            // data.at(pinIndex).addPlotDataItem("Latitude [deg]");
            // data.at(pinIndex).addPlotDataItem("Longitude [deg]");
            // data.at(pinIndex).addPlotDataItem("Altitude [m]");
            // data.at(pinIndex).addPlotDataItem("North/South [m]");
