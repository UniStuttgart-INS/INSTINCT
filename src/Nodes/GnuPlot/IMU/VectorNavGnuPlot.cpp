...
NAV::NavStatus NAV::VectorNavGnuPlot::plotVectorNavObs(std::shared_ptr<NAV::NodeData> observation, std::shared_ptr<NAV::Node> userData)
{
    ...
    if (!obj->insTime0.has_value())
    {
        if (obs->insTime.has_value())
        {
            obj->insTime0 = obs->insTime;
            // Clear previous data, as time base changed
            for (auto& [xData, yData, wIndex, dataIndices] : obj->dataToPlot)
                for (auto& dataIndex : dataIndices)
                    plotWindows.at(wIndex)->data.at(dataIndex).xy.clear();
        }
        else if (!obj->timeSinceStartup0)
            obj->timeSinceStartup0 = obs->timeSinceStartup.value();
    }

    double plotX;
    if (obj->insTime0.has_value())
        plotX = static_cast<double>(obs->insTime.value().getTimeDiff(obj->insTime0.value()));
    else
        plotX = static_cast<double>(obs->timeSinceStartup.value() - obj->timeSinceStartup0) / std::pow(10, 9);
    ...
    for (auto& [xData, yData, wIndex, dataIndices] : obj->dataToPlot)
    {
        ...
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->timeSinceStartup.value()));
        }
        else if (yData == "quaternion" && obs->quaternion.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->quaternion.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->quaternion.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->quaternion.value().z()));
            plotWindow->data.at(dataIndices.at(3)).xy.push_back(std::make_pair(plotX, obs->quaternion.value().w()));
        }
        else if (yData == "magUncompXYZ" && obs->magUncompXYZ.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->magUncompXYZ.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->magUncompXYZ.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->magUncompXYZ.value().z()));
        }
        else if (yData == "accelUncompXYZ" && obs->accelUncompXYZ.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->accelUncompXYZ.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->accelUncompXYZ.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->accelUncompXYZ.value().z()));
        }
        else if (yData == "gyroUncompXYZ" && obs->gyroUncompXYZ.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->gyroUncompXYZ.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->gyroUncompXYZ.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->gyroUncompXYZ.value().z()));
        }
        else if (yData == "magCompXYZ" && obs->magCompXYZ.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->magCompXYZ.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->magCompXYZ.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->magCompXYZ.value().z()));
        }
        else if (yData == "accelCompXYZ" && obs->accelCompXYZ.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->accelCompXYZ.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->accelCompXYZ.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->accelCompXYZ.value().z()));
        }
        else if (yData == "gyroCompXYZ" && obs->gyroCompXYZ.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->gyroCompXYZ.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->gyroCompXYZ.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->gyroCompXYZ.value().z()));
        }
        else if (yData == "syncInCnt" && obs->syncInCnt.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->syncInCnt.value()));
        }
        else if (yData == "dtime" && obs->dtime.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->dtime.value()));
        }
        else if (yData == "dtheta" && obs->dtheta.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->dtheta.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->dtheta.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->dtheta.value().z()));
        }
        else if (yData == "dvel" && obs->dvel.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->dvel.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->dvel.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->dvel.value().z()));
        }
        else if (yData == "vpeStatus" && obs->vpeStatus.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->vpeStatus.value()));
        }
        else if (yData == "temperature" && obs->temperature.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->temperature.value()));
        }
        else if (yData == "pressure" && obs->pressure.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->pressure.value()));
        }
        else if (yData == "magCompNED" && obs->magCompNED.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->magCompNED.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->magCompNED.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->magCompNED.value().z()));
        }
        else if (yData == "accelCompNED" && obs->accelCompNED.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->accelCompNED.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->accelCompNED.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->accelCompNED.value().z()));
        }
        else if (yData == "gyroCompNED" && obs->gyroCompNED.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->gyroCompNED.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->gyroCompNED.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->gyroCompNED.value().z()));
        }
        else if (yData == "linearAccelXYZ" && obs->linearAccelXYZ.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->linearAccelXYZ.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->linearAccelXYZ.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->linearAccelXYZ.value().z()));
        }
        else if (yData == "linearAccelNED" && obs->linearAccelNED.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->linearAccelNED.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->linearAccelNED.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->linearAccelNED.value().z()));
        }
        else if (yData == "yawPitchRollUncertainty" && obs->yawPitchRollUncertainty.has_value())
        {
            plotWindow->data.at(dataIndices.at(0)).xy.push_back(std::make_pair(plotX, obs->yawPitchRollUncertainty.value().x()));
            plotWindow->data.at(dataIndices.at(1)).xy.push_back(std::make_pair(plotX, obs->yawPitchRollUncertainty.value().y()));
            plotWindow->data.at(dataIndices.at(2)).xy.push_back(std::make_pair(plotX, obs->yawPitchRollUncertainty.value().z()));
        }

        // Delete old data
        for (auto& dataIndex : dataIndices)
        {
            while (obj->timeFrame && plotWindow->data.at(dataIndex).xy.back().first - plotWindow->data.at(dataIndex).xy.front().first > obj->timeFrame)
                plotWindow->data.at(dataIndex).xy.pop_front();
        }
    }

    if (requestUpdate(obj) != NavStatus::NAV_OK)
        LOG_ERROR("Something went wrong while plotting the data");

    return obj->invokeCallbacks(NodeInterface::getCallbackPort("VectorNavGnuPlot", "VectorNavObs"), observation);
}