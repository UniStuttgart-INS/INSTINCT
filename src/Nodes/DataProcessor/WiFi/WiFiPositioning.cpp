    ...
    : Node(typeStatic())
{
    ...
    _hasConfig = true;
    _guiConfigDefaultWindowSize = { 600, 500 };
...
void NAV::WiFiPositioning::guiConfig()
{
    ...
    if (ImGui::BeginTable("AccessPointInput", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_NoHostExtendX, ImVec2(0.0F, 0.0F)))
    {
        ...
        for (size_t rowIndex = 0; rowIndex < _numOfDevices; rowIndex++)
        {
            ...
            auto* devPosRow = _devicePositions.at(rowIndex).data();
            ...
            if (_frame == Frame::ECEF)
            {
                ImGui::TableNextColumn();
                ImGui::SetNextItemWidth(columnWidth);
                if (ImGui::InputDouble(fmt::format("##InputX{}", rowIndex).c_str(), &devPosRow[0], 0.0, 0.0, "%.4fm"))
                {
                    flow::ApplyChanges();
                }
                ImGui::TableNextColumn();
                ImGui::SetNextItemWidth(columnWidth);
                if (ImGui::InputDouble(fmt::format("##InputY{}", rowIndex).c_str(), &devPosRow[1], 0.0, 0.0, "%.4fm"))
                {
                    flow::ApplyChanges();
                }
                ImGui::TableNextColumn();
                ImGui::SetNextItemWidth(columnWidth);
                if (ImGui::InputDouble(fmt::format("##InputZ{}", rowIndex).c_str(), &devPosRow[2], 0.0, 0.0, "%.4fm"))
                {
                    flow::ApplyChanges();
                }
            }
            else if (_frame == Frame::LLA)
            {
                ImGui::TableNextColumn();
                ImGui::SetNextItemWidth(columnWidth);
                if (ImGui::InputDoubleL(fmt::format("##InputLat{}", rowIndex).c_str(), &devPosRow[0], -180, 180, 0.0, 0.0, "%.8f°"))
                {
                    flow::ApplyChanges();
                }
                ImGui::TableNextColumn();
                ImGui::SetNextItemWidth(columnWidth);
                if (ImGui::InputDoubleL(fmt::format("##InputLon{}", rowIndex).c_str(), &devPosRow[1], -180, 180, 0.0, 0.0, "%.8f°"))
                {
                    flow::ApplyChanges();
                }
                ImGui::TableNextColumn();
                ImGui::SetNextItemWidth(columnWidth);
                if (ImGui::InputDouble(fmt::format("##InputHeight{}", rowIndex).c_str(), &devPosRow[2], 0.0, 0.0, "%.4fm"))
                {
                    flow::ApplyChanges();
                }
            }
    ...
    {
        _numOfDevices++;
        _deviceMacAddresses.emplace_back("00:00:00:00:00:00");
        _devicePositions.emplace_back(Eigen::Vector3d::Zero());
        _deviceBias.emplace_back(0.0);
        _deviceScale.emplace_back(0.0);
        flow::ApplyChanges();
    }
    ImGui::SameLine();
...
void NAV::WiFiPositioning::recvWiFiObs(NAV::InputPin::NodeDataQueue& queue, size_t /* pinIdx */)
{
    ...
    if (it != _deviceMacAddresses.end()) // Check if the MAC address is in the list
    {
        ...
        auto index = static_cast<size_t>(std::distance(_deviceMacAddresses.begin(), it));
