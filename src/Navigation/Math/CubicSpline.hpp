...
class CubicSpline
{
  ...
  public:
    CubicSpline() : boundary_type_left(second_deriv), boundary_type_right(second_deriv), boundary_value_left(0.0), boundary_value_right(0.0)
    {
        ;
    }
    CubicSpline(const std::vector<double>& X, const std::vector<double>& Y,
                boundary_derivative_type left = second_deriv, double left_value = 0.0,
                boundary_derivative_type right = second_deriv, double right_value = 0.0) : boundary_type_left(left), boundary_type_right(right), boundary_value_left(left_value), boundary_value_right(right_value)
    {
        this->set_points(X, Y);
    }
