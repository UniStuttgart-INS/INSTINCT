...
/// Possible Units for the Standard deviation of the jerk due to user motion
enum class CovarianceJerkUnits : uint8_t
{
    m2_s5,    ///< [ m^2 / s^5 ]
    m_sqrts5, ///< [ m / √(s^5) ]
    COUNT,    ///< Amount of items in the enum
...
/// @param[in] unit Unit the value is in
/// @return Value in unit of the first item in the Unit enum
[[nodiscard]] double convertUnit(const double& value, Units::CovarianceJerkUnits unit);
...
/// @brief Converts the unit into a string
/// @param[in] unit Unit
[[nodiscard]] std::string to_string(Units::CovarianceJerkUnits unit);
/// @brief Converts the unit into a string
/// @param[in] unit Unit
