...
GPSEphemeris::GPSEphemeris(int32_t year, int32_t month, int32_t day, int32_t hour, int32_t minute, double second, double svClockBias, double svClockDrift, double svClockDriftRate,
                           ...
                           double /* TransmissionTimeOfMessage */, double fitInterval, double /* spare1 */, double /* spare2 */)
    : SatNavData(SatNavData::GPSEphemeris, InsTime(year, month, day, hour, minute, second, SatelliteSystem(GPS).getTimeSystem())),
      toc(refTime),
      toe(InsTime(0, static_cast<int32_t>(GPSWeek), Toe, SatelliteSystem(GPS).getTimeSystem())),
...
Orbit::Pos GPSEphemeris::calcSatellitePos(const InsTime& transTime) const
{
    ...
    return { .e_pos = Eigen::Vector3d{ x_k, y_k, z_k } };
}

Orbit::PosVel GPSEphemeris::calcSatellitePosVel(const InsTime& transTime) const
{
    LOG_DATA("Calc Sat Position at transmit time {}", transTime.toGPSweekTow());
    // Earth gravitational constant [m³/s²] (WGS 84 value of the earth's gravitational constant for GPS user)
    const auto mu = InsConst::GPS::MU;
...
Orbit::PosVelAccel GPSEphemeris::calcSatellitePosVelAccel(const InsTime& transTime) const
{
    LOG_DATA("Calc Sat Position at transmit time {}", transTime.toGPSweekTow());
    // Earth gravitational constant [m³/s²] (WGS 84 value of the earth's gravitational constant for GPS user)
    const auto mu = InsConst::GPS::MU;
...
Clock::Corrections GPSEphemeris::calcClockCorrections(const InsTime& recvTime, double dist, const Frequency& freq) const
{
...
bool GPSEphemeris::isHealthy() const
{
    return svHealth == 0;
}
