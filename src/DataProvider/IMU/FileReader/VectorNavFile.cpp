...
std::shared_ptr<NAV::InsObs> NAV::VectorNavFile::pollObservation()
{
    ...
    else
    {
        ...
        Vector<3, double> ypr(NAN);
        Matrix<3, 3, double> dcm(NAN);
            ...
            if (std::getline(lineStream, cell, ','))
            {
                ...
                else if (columns[i] == "UnCompMagX")
                {
                    if (!obs->magUncompXYZ.has_value())
                        obs->magUncompXYZ = Vector<3, double>();
                ...
                else if (columns[i] == "UnCompAccX")
                {
                    if (!obs->accelUncompXYZ.has_value())
                        obs->accelUncompXYZ = Vector<3, double>();
                ...
                else if (columns[i] == "UnCompGyroX")
                {
                    if (!obs->gyroUncompXYZ.has_value())
                        obs->gyroUncompXYZ = Vector<3, double>();
                ...
                else if (columns[i] == "MagX")
                {
                    if (!obs->magCompXYZ.has_value())
                        obs->magCompXYZ = Vector<3, double>();
                ...
                else if (columns[i] == "AccX")
                {
                    if (!obs->accelCompXYZ.has_value())
                        obs->accelCompXYZ = Vector<3, double>();
                ...
                else if (columns[i] == "GyroX")
                {
                    if (!obs->gyroCompXYZ.has_value())
                        obs->gyroCompXYZ = Vector<3, double>();
                ...
                else if (columns[i] == "Quat[0]")
                {
                    if (!obs->quaternion.has_value())
                        obs->quaternion = Quaternion<double>();
                    obs->quaternion.value()(1) = std::stof(cell);
                }
                else if (columns[i] == "Quat[1]")
                {
                    if (!obs->quaternion.has_value())
                        obs->quaternion = Quaternion<double>();
                    obs->quaternion.value()(2) = std::stof(cell);
                }
                else if (columns[i] == "Quat[2]")
                {
                    if (!obs->quaternion.has_value())
                        obs->quaternion = Quaternion<double>();
                    obs->quaternion.value()(3) = std::stof(cell);
                }
                else if (columns[i] == "Quat[3]")
                {
                    if (!obs->quaternion.has_value())
                        obs->quaternion = Quaternion<double>();
                    obs->quaternion.value()(0) = std::stof(cell);
                }
                ...
                else if (columns[i] == "MagN")
                {
                    if (!obs->magCompNED.has_value())
                        obs->magCompNED = Vector<3, double>();
                    obs->magCompNED.value().n = std::stof(cell);
                }
                else if (columns[i] == "MagE")
                {
                    if (!obs->magCompNED.has_value())
                        obs->magCompNED = Vector<3, double>();
                    obs->magCompNED.value().e = std::stof(cell);
                }
                else if (columns[i] == "MagD")
                {
                    if (!obs->magCompNED.has_value())
                        obs->magCompNED = Vector<3, double>();
                    obs->magCompNED.value().d = std::stof(cell);
                }
                else if (columns[i] == "AccN")
                {
                    if (!obs->accelCompNED.has_value())
                        obs->accelCompNED = Vector<3, double>();
                    obs->accelCompNED.value().n = std::stof(cell);
                }
                else if (columns[i] == "AccE")
                {
                    if (!obs->accelCompNED.has_value())
                        obs->accelCompNED = Vector<3, double>();
                    obs->accelCompNED.value().e = std::stof(cell);
                }
                else if (columns[i] == "AccD")
                {
                    if (!obs->accelCompNED.has_value())
                        obs->accelCompNED = Vector<3, double>();
                    obs->accelCompNED.value().d = std::stof(cell);
                }
                else if (columns[i] == "LinAccX")
                {
                    if (!obs->linearAccelXYZ.has_value())
                        obs->linearAccelXYZ = Vector<3, double>();
                ...
                else if (columns[i] == "LinAccN")
                {
                    if (!obs->linearAccelNED.has_value())
                        obs->linearAccelNED = Vector<3, double>();
                    obs->linearAccelNED.value().n = std::stof(cell);
                }
                else if (columns[i] == "LinAccE")
                {
                    if (!obs->linearAccelNED.has_value())
                        obs->linearAccelNED = Vector<3, double>();
                    obs->linearAccelNED.value().e = std::stof(cell);
                }
                else if (columns[i] == "LinAccD")
                {
                    if (!obs->linearAccelNED.has_value())
                        obs->linearAccelNED = Vector<3, double>();
                    obs->linearAccelNED.value().d = std::stof(cell);
                }
                else if (columns[i] == "YawU")
                {
                    if (!obs->yawPitchRollUncertainty.has_value())
                        obs->yawPitchRollUncertainty = Vector<3, double>();
                ...
                else if (columns[i] == "YawRate")
                {
                    if (!obs->gyroCompNED.has_value())
                        obs->gyroCompNED = Vector<3, double>();
                    obs->gyroCompNED.value().n = std::stof(cell);
                }
                else if (columns[i] == "PitchRate")
                {
                    if (!obs->gyroCompNED.has_value())
                        obs->gyroCompNED = Vector<3, double>();
                    obs->gyroCompNED.value().e = std::stof(cell);
                }
                else if (columns[i] == "RollRate")
                {
                    if (!obs->gyroCompNED.has_value())
                        obs->gyroCompNED = Vector<3, double>();
                    obs->gyroCompNED.value().d = std::stof(cell);
                }
            }
            else
        ...
        if (!obs->quaternion.has_value())
        {
            if (!std::isnan(dcm(0, 0)))
                obs->quaternion = Quaternion<double>::dcm2quat(dcm);
            else if (!std::isnan(ypr(0)))
            {
                dcm = Matrix<3, 3, double>::rotXYZ_deg(ypr(0), ypr(1), ypr(2));
                obs->quaternion = Quaternion<double>::dcm2quat(dcm);
            }
        }
