...
TEST_CASE("[InsMechanization] PVAError correction", "[InsMechanization]")
{
    ...
    double pitch = trafo::deg2rad(-4.0);
    ...
    Eigen::Vector3d attitudeError_n{ trafo::deg2rad3({ 5, 2, -1 }) };
    ...
    CHECK(posVelAtt->latLonAlt()(0) == Approx(correctedPVA->latLonAlt()(0) + positionError_lla(0)).margin(1e-16));
    CHECK(posVelAtt->latLonAlt()(1) == Approx(correctedPVA->latLonAlt()(1) + positionError_lla(1)).margin(1e-16));
    CHECK(posVelAtt->latLonAlt()(2) == Approx(correctedPVA->latLonAlt()(2) + positionError_lla(2)).margin(1e-16));

    CHECK(posVelAtt->velocity_n()(0) == Approx(correctedPVA->velocity_n()(0) + velocityError_n(0)).margin(1e-16));
    CHECK(posVelAtt->velocity_n()(1) == Approx(correctedPVA->velocity_n()(1) + velocityError_n(1)).margin(1e-16));
    CHECK(posVelAtt->velocity_n()(2) == Approx(correctedPVA->velocity_n()(2) + velocityError_n(2)).margin(1e-16));

    CHECK(trafo::rad2deg(posVelAtt->rollPitchYaw()(0)) == Approx(trafo::rad2deg(correctedPVA->rollPitchYaw()(0) + attitudeError_n(0))));
    CHECK(trafo::rad2deg(posVelAtt->rollPitchYaw()(1)) == Approx(trafo::rad2deg(correctedPVA->rollPitchYaw()(1) + attitudeError_n(1))));
    CHECK(trafo::rad2deg(posVelAtt->rollPitchYaw()(2)) == Approx(trafo::rad2deg(correctedPVA->rollPitchYaw()(2) + attitudeError_n(2))));
}
